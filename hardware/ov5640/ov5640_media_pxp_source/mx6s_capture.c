...
static void csi_init_interface(struct mx6s_csi_dev *csi_dev)
{
	...
	/* v3.27: remove SWAP16, sensor YUYV → mem YUYV directly */
	val |= BIT_SOF_POL;
	val |= BIT_REDGE;
	val |= BIT_GCLK_MODE;
	val |= BIT_HSYNC_POL;
	val |= BIT_FCC;
	/* BIT_SWAP16_EN removed - sensor outputs standard YUYV */
	val |= 1 << SHIFT_MCLKDIV;
	val |= BIT_MCLKEN;
...
static void csi_tvdec_enable(struct mx6s_csi_dev *csi_dev, bool enable)
{
	...
	if (enable == true) {
		...
		cr18 &= ~(BIT_TVDECODER_IN_EN |
				...
				BIT_BASEADDR_CHG_ERR_EN);
		cr1 &= ~BIT_CCIR_MODE;
		cr1 |= BIT_REDGE;   /* SOF_POL removed: align with STM32 DCMI (VSYNC low) */
	}
			       ...
			       enum v4l2_buf_type i)
{
	...
	/* Start sensor first, so CSI can see SOF immediately */
...
static int subdev_notifier_complete(struct v4l2_async_notifier *notifier)
{
	...
	/* Create media link: sensor source pad(0) → CSI sink pad(0)
	 * Done here in complete callback because both entities are
	 * guaranteed to be fully initialized at this point.
	 */
	if (csi_dev->sd && csi_dev->vdev) {
		ret = media_create_pad_link(
			&csi_dev->sd->entity, 0,	/* sensor source pad */
			&csi_dev->vdev->entity, 0,	/* CSI sink pad */
			MEDIA_LNK_FL_ENABLED |
			MEDIA_LNK_FL_IMMUTABLE);
		if (ret)
			v4l2_err(&csi_dev->v4l2_dev,
				 "failed to create media link: %d\n", ret);
		else
			v4l2_info(&csi_dev->v4l2_dev,
				  "media link created: %s:0 -> %s:0\n",
				  csi_dev->sd->name, csi_dev->vdev->name);
	}
...
static int mx6s_csi_probe(struct platform_device *pdev)
{
...
#ifdef CONFIG_MEDIA_CONTROLLER
	/* Initialize media device and register it */
	csi_dev->mdev.dev = dev;
	strlcpy(csi_dev->mdev.model, "i.MX6S_CSI",
		sizeof(csi_dev->mdev.model));
	media_device_init(&csi_dev->mdev);
	ret = media_device_register(&csi_dev->mdev);
	if (ret < 0) {
		dev_err(dev, "media_device_register failed: %d\n", ret);
		v4l2_device_unregister(&csi_dev->v4l2_dev);
		return ret;
	}
	csi_dev->v4l2_dev.mdev = &csi_dev->mdev;
#endif
...
#ifdef CONFIG_MEDIA_CONTROLLER
	/* Set up CSI video device as media entity with a sink pad */
	csi_dev->pad.flags = MEDIA_PAD_FL_SINK;
	ret = media_entity_pads_init(&vdev->entity, 1, &csi_dev->pad);
	if (ret < 0) {
	...
	pr_info("mx6s_capture v3.27 FINAL: no SWAP16, std YUYV, color OK\n");
