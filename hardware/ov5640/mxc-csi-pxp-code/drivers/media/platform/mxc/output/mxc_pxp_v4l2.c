...
#define PXP_SD_PAD_SINK		0
#define PXP_SD_PAD_SOURCE	1
#define PXP_SD_PADS_NUM		2
...
static int pxp_find_fbinfo_by_node(int fb_node, struct fb_info **fbi)
{
	int i;
	...
	for (i = 0; i < num_registered_fb; i++) {
		if (!registered_fb[i])
			continue;
		if (registered_fb[i]->node == fb_node) {
			*fbi = registered_fb[i];
			return 0;
		}
	}
...
static int pxp_get_fbinfo(struct pxps *pxp, struct fb_info **fbi)
{
	int i;
	char *idstr;

	if (pxp->fb_node >= 0)
		return pxp_find_fbinfo_by_node(pxp->fb_node, fbi);

	for (i = 0; i < num_registered_fb; i++) {
		if (!registered_fb[i])
			continue;
		idstr = registered_fb[i]->fix.id;
...
static int pxp_set_fbinfo(struct pxps *pxp)
{
	...
	err = pxp_get_fbinfo(pxp, &pxp->fbi);
	if (err)
		return err;
...
static int pxp_apply_output(struct pxps *pxp, unsigned int output)
{
...
static int set_fb_blank(struct pxps *pxp, int blank)
{
	struct fb_info *fbi;
	int err = 0;
			...
			enum v4l2_buf_type t)
{
	...
	set_fb_blank(pxp, FB_BLANK_UNBLANK);
			...
			enum v4l2_buf_type t)
{
	...
	if (pxp->fb_blank)
		set_fb_blank(pxp, FB_BLANK_POWERDOWN);
...
static long pxp_ioctl_default(struct file *file, void *fh, bool valid_prio,
			      unsigned int cmd, void *arg)
{
	struct pxps *pxp = video_get_drvdata(video_devdata(file));
	struct pxp_video_output_fb *fb_cfg = arg;
	int old_fb_node;
	int ret = 0;

	switch (cmd) {
	case PXP_IOC_V4L2_S_OUTPUT_FB:
		if (!fb_cfg)
			return -EINVAL;
		if (fb_cfg->fb_index < -1)
			return -EINVAL;

		mutex_lock(&pxp->mutex);
		if (pxp->active || !list_empty(&pxp->outq)) {
			ret = -EBUSY;
			goto unlock;
		}

		old_fb_node = pxp->fb_node;
		pxp->fb_node = fb_cfg->fb_index;
		ret = pxp_set_fbinfo(pxp);
		if (ret < 0) {
			pxp->fb_node = old_fb_node;
			pxp_set_fbinfo(pxp);
			goto unlock;
		}

		ret = pxp_apply_output(pxp, pxp->output);
unlock:
		mutex_unlock(&pxp->mutex);
		return ret;

	case PXP_IOC_V4L2_G_OUTPUT_FB:
		if (!fb_cfg)
			return -EINVAL;
		memset(fb_cfg, 0, sizeof(*fb_cfg));
		if (pxp->fbi)
			fb_cfg->fb_index = pxp->fbi->node;
		else
			fb_cfg->fb_index = pxp->fb_node;
		return 0;
	default:
		return -ENOTTY;
	}
...
static const struct v4l2_ioctl_ops pxp_ioctl_ops = {
	...
	.vidioc_default			= pxp_ioctl_default,
};
...
static int pxp_sd_s_stream(struct v4l2_subdev *sd, int enable)
{
	return 0;
}

static const struct v4l2_subdev_video_ops pxp_sd_video_ops = {
	.s_stream = pxp_sd_s_stream,
};

static const struct v4l2_subdev_ops pxp_sd_ops = {
	.video = &pxp_sd_video_ops,
};

static int pxp_create_internal_link(struct pxps *pxp)
{
	int ret;

	ret = media_create_pad_link(&pxp->vdev->entity, 0,
				    &pxp->pxp_sd.entity, PXP_SD_PAD_SINK,
	...
	if (ret < 0)
		dev_err(&pxp->pdev->dev,
			"failed to create internal PXP media link: %d\n", ret);
...
#if defined(CONFIG_MEDIA_CONTROLLER)
	pxp->media_dev = mxc_get_shared_media_device(&pdev->dev);
	v4l2_dev->mdev = pxp->media_dev;
#endif
...
#if defined(CONFIG_MEDIA_CONTROLLER)
		mxc_put_shared_media_device(pxp->media_dev);
#endif
		goto freev4l2;
	}
...
#if defined(CONFIG_MEDIA_CONTROLLER)
	v4l2_subdev_init(&pxp->pxp_sd, &pxp_sd_ops);
	pxp->pxp_sd.owner = THIS_MODULE;
	pxp->pxp_sd.dev = &pdev->dev;
	strlcpy(pxp->pxp_sd.name, "pxp-subdev", sizeof(pxp->pxp_sd.name));
	pxp->pxp_sd.flags |= V4L2_SUBDEV_FL_HAS_DEVNODE;
	pxp->pxp_sd.entity.function = MEDIA_ENT_F_PROC_VIDEO_PIXEL_FORMATTER;
	pxp->sd_pads[PXP_SD_PAD_SINK].flags = MEDIA_PAD_FL_SINK;
	pxp->sd_pads[PXP_SD_PAD_SOURCE].flags = MEDIA_PAD_FL_SOURCE;
	err = media_entity_pads_init(&pxp->pxp_sd.entity, PXP_SD_PADS_NUM,
				     pxp->sd_pads);
	if (err) {
		dev_err(&pdev->dev, "failed to init pxp subdev pads\n");
		media_entity_cleanup(&pxp->pxp_sd.entity);
		video_device_release(pxp->vdev);
		goto relv4l2;
	}
	v4l2_set_subdevdata(&pxp->pxp_sd, pxp);
	err = v4l2_device_register_subdev(v4l2_dev, &pxp->pxp_sd);
	if (err) {
		dev_err(&pdev->dev, "failed to register pxp subdev\n");
		media_entity_cleanup(&pxp->pxp_sd.entity);
		video_device_release(pxp->vdev);
		goto relv4l2;
	}
	pxp->pxp_sd_registered = true;

	pxp->vdev_pad.flags = MEDIA_PAD_FL_SOURCE;
	err = media_entity_pads_init(&pxp->vdev->entity, 1, &pxp->vdev_pad);
	if (err) {
		dev_err(&pdev->dev, "failed to init pxp media pad\n");
		v4l2_device_unregister_subdev(&pxp->pxp_sd);
		pxp->pxp_sd_registered = false;
		media_entity_cleanup(&pxp->pxp_sd.entity);
		video_device_release(pxp->vdev);
		goto relv4l2;
	}
...
#if defined(CONFIG_MEDIA_CONTROLLER)
	err = pxp_create_internal_link(pxp);
	if (err)
		goto unregvdev;

	err = v4l2_device_register_subdev_nodes(v4l2_dev);
	if (err < 0)
		dev_warn(&pdev->dev, "failed to register pxp subdev nodes: %d\n",
			 err);

	err = mxc_register_shared_media_device(pxp->media_dev);
	if (err) {
...
unregvdev:
	...
	media_entity_cleanup(&pxp->vdev->entity);
	if (pxp->pxp_sd_registered) {
		v4l2_device_unregister_subdev(&pxp->pxp_sd);
		pxp->pxp_sd_registered = false;
		media_entity_cleanup(&pxp->pxp_sd.entity);
	}
