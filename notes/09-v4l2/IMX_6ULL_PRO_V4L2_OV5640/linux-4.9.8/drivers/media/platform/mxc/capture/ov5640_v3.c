 ...
 */
 ...
 static struct reg_value ov5640_config[] = {
	 ...
	 {0x4740, 0x21, 0, 0}, /* BIT5: Pixel BIT1: HSYNC BIT0: VSYNC
							  1: hight level
							  0: low level valid*/
	 {0x5000, 0xa7, 0, 0},
 };
...
static const struct ov5640_datafmt ov5640_colour_fmts[] = {
	{MEDIA_BUS_FMT_YUYV8_2X8, V4L2_PIX_FMT_YUYV, 0x30, V4L2_COLORSPACE_JPEG},
	{MEDIA_BUS_FMT_YVYU8_2X8, V4L2_PIX_FMT_YVYU, 0x31, V4L2_COLORSPACE_JPEG},
	{MEDIA_BUS_FMT_UYVY8_2X8, V4L2_PIX_FMT_UYVY, 0x32, V4L2_COLORSPACE_JPEG},
 };
...
static int ov5640_apply_datafmt(struct ov5640 *sensor)
{
	const struct ov5640_datafmt *fmt = sensor->fmt;

	if (!fmt)
		fmt = sensor->mipi_csi ? &ov5640_colour_fmts[2] :
					 &ov5640_colour_fmts[0];

	ov5640_write_reg(0x4300, fmt->reg4300);
	ov5640_write_reg(0x501f, 0x00);
	sensor->pix.pixelformat = fmt->pixelformat;

    pr_debug("ov5640_apply_datafmt,reg = %d, pixelformat = %ld \n", fmt->reg4300,fmt->pixelformat);
 ...
 static int ov5640_config_init(void)
 {
	...
	retval = ov5640_apply_datafmt(&ov5640_data);
	if (retval < 0)
		return retval;
			   ...
			   struct v4l2_subdev_format *format)
 {
     ...
     pr_debug(" ov5640_set_fmt  mf->code - %d\n", mf->code);
	 if (!fmt) {
		fmt = sensor->fmt ? sensor->fmt :
		      (sensor->mipi_csi ? &ov5640_colour_fmts[2] :
					  &ov5640_colour_fmts[0]);
		mf->code = fmt->code;
...
static const struct media_entity_operations ov5640_sd_media_ops = {
			 ...
			 const struct i2c_device_id *id)
 {
	...
	ov5640_data.fmt = ov5640_data.mipi_csi ? &ov5640_colour_fmts[2] :
					       &ov5640_colour_fmts[0];
	ov5640_data.pix.pixelformat = ov5640_data.fmt->pixelformat;
