...
#define MX6S_CSI_VDEV_PADS_NUM 1
#define MX6S_CSI_SD_PAD_SINK 0
#define MX6S_CSI_SD_PAD_SOURCE 1
#define MX6S_CSI_SD_PADS_NUM 2
...
struct mx6s_csi_dev {
	...
	struct media_pad csi_sd_pads[MX6S_CSI_SD_PADS_NUM];
	struct media_pad vdev_pad;
	bool csi_sd_registered;
#endif
...
static inline struct mx6s_csi_dev *sd_to_mx6s_csi_dev(struct v4l2_subdev *sd)
{
	return v4l2_get_subdevdata(sd);
}

static int mx6s_csi_sd_enum_mbus_code(struct v4l2_subdev *sd,
				      struct v4l2_subdev_pad_config *cfg,
				      struct v4l2_subdev_mbus_code_enum *code)
{
	if (code->pad >= MX6S_CSI_SD_PADS_NUM || code->index >= NUM_FORMATS)
		return -EINVAL;
...
static int mx6s_csi_sd_get_fmt(struct v4l2_subdev *sd,
			       struct v4l2_subdev_pad_config *cfg,
			       struct v4l2_subdev_format *format)
{
	struct mx6s_csi_dev *csi_dev = sd_to_mx6s_csi_dev(sd);

	if (format->pad >= MX6S_CSI_SD_PADS_NUM)
		return -EINVAL;

	if (!csi_dev->fmt)
		csi_dev->fmt = &formats[0];
	if (!csi_dev->mbus_code)
		csi_dev->mbus_code = csi_dev->fmt->mbus_code;

	format->format.code = csi_dev->mbus_code;
	format->format.width = csi_dev->pix.width ? csi_dev->pix.width : 640;
	format->format.height = csi_dev->pix.height ? csi_dev->pix.height : 480;
	format->format.field = V4L2_FIELD_NONE;
	format->format.colorspace = csi_dev->pix.colorspace ?
				    csi_dev->pix.colorspace :
				    V4L2_COLORSPACE_JPEG;

	return 0;
}

static int mx6s_csi_sd_set_fmt(struct v4l2_subdev *sd,
			       struct v4l2_subdev_pad_config *cfg,
			       struct v4l2_subdev_format *format)
{
	struct mx6s_csi_dev *csi_dev = sd_to_mx6s_csi_dev(sd);
	...
	if (!fmt)
		fmt = csi_dev->fmt ? csi_dev->fmt : &formats[0];
	...
	csi_dev->pix.width = format->format.width;
	csi_dev->pix.height = format->format.height;
	csi_dev->pix.colorspace = format->format.colorspace;

	return 0;
}

static int mx6s_csi_sd_s_stream(struct v4l2_subdev *sd, int enable)
{
	return 0;
}

static const struct v4l2_subdev_pad_ops mx6s_csi_sd_pad_ops = {
	.enum_mbus_code = mx6s_csi_sd_enum_mbus_code,
	.get_fmt = mx6s_csi_sd_get_fmt,
	.set_fmt = mx6s_csi_sd_set_fmt,
};

static const struct v4l2_subdev_video_ops mx6s_csi_sd_video_ops = {
	.s_stream = mx6s_csi_sd_s_stream,
};

static const struct v4l2_subdev_ops mx6s_csi_sd_ops = {
	.video = &mx6s_csi_sd_video_ops,
	.pad = &mx6s_csi_sd_pad_ops,
};

static int mx6s_csi_create_sensor_link(struct mx6s_csi_dev *csi_dev,
				       struct v4l2_subdev *subdev)
{
	...
	if (!source->pads || !source->num_pads) {
		dev_warn(csi_dev->dev,
			 "subdev %s has no media pads, skip link creation\n",
			 subdev->name);
		return 0;
	}
	...
	for (i = 0; i < source->num_pads; i++) {
		...
		ret = media_create_pad_link(source, i, &csi_dev->csi_sd.entity,
					    MX6S_CSI_SD_PAD_SINK,
		...
		if (ret < 0) {
			dev_err(csi_dev->dev,
				"failed to create media link for %s: %d\n",
				subdev->name, ret);
			return ret;
		}
	...
	dev_warn(csi_dev->dev, "subdev %s exposes no source pad\n", subdev->name);
	return 0;
}

static int mx6s_csi_create_capture_link(struct mx6s_csi_dev *csi_dev)
{
	int ret;

	ret = media_create_pad_link(&csi_dev->csi_sd.entity,
				    MX6S_CSI_SD_PAD_SOURCE,
				    &csi_dev->vdev->entity, 0,
				    MEDIA_LNK_FL_ENABLED |
				    MEDIA_LNK_FL_IMMUTABLE);
	if (ret == -EEXIST)
		return 0;
	if (ret < 0) {
		dev_err(csi_dev->dev,
			"failed to create csi-to-capture link: %d\n", ret);
		return ret;
	}
...
#if defined(CONFIG_MEDIA_CONTROLLER)
	if (csi_dev->sd) {
		ret = mx6s_csi_create_sensor_link(csi_dev, csi_dev->sd);
		if (ret < 0)
			return ret;
	}
#endif

	ret = v4l2_device_register_subdev_nodes(&csi_dev->v4l2_dev);
	if (ret < 0)
		dev_warn(csi_dev->dev, "failed to register subdev nodes: %d\n",
			 ret);

	ret = mxc_register_shared_media_device(csi_dev->media_dev);
	if (ret < 0) {
		dev_err(csi_dev->dev, "failed to register media device: %d\n",
			ret);
		return ret;
	}
	return 0;
...
static void csi_init_interface(struct mx6s_csi_dev *csi_dev)
{
	...
	/*
	 * OV5640 DVP (0x4740=0x23): VSYNC/HREF active high -> clear SOF/HSYNC_POL.
	 */
	val |= BIT_REDGE;
	val |= BIT_GCLK_MODE;
	val |= BIT_FCC;
...
#if defined(CONFIG_MEDIA_CONTROLLER)
	csi_dev->subdev_notifier.complete = subdev_notifier_complete;
#endif
...
#if defined(CONFIG_MEDIA_CONTROLLER)
	csi_dev->media_dev = mxc_get_shared_media_device(dev);
	csi_dev->v4l2_dev.mdev = csi_dev->media_dev;
#endif
...
#if defined(CONFIG_MEDIA_CONTROLLER)
		mxc_put_shared_media_device(csi_dev->media_dev);
#endif
		return -ENODEV;
	}
...
#if defined(CONFIG_MEDIA_CONTROLLER)
	v4l2_subdev_init(&csi_dev->csi_sd, &mx6s_csi_sd_ops);
	csi_dev->csi_sd.owner = THIS_MODULE;
	csi_dev->csi_sd.dev = dev;
	strlcpy(csi_dev->csi_sd.name, "mx6s-csi-subdev",
		sizeof(csi_dev->csi_sd.name));
	csi_dev->csi_sd.flags |= V4L2_SUBDEV_FL_HAS_DEVNODE;
	csi_dev->csi_sd.entity.function = MEDIA_ENT_F_IO_V4L;
	csi_dev->csi_sd_pads[MX6S_CSI_SD_PAD_SINK].flags = MEDIA_PAD_FL_SINK;
	csi_dev->csi_sd_pads[MX6S_CSI_SD_PAD_SOURCE].flags = MEDIA_PAD_FL_SOURCE;
	ret = media_entity_pads_init(&csi_dev->csi_sd.entity,
				     MX6S_CSI_SD_PADS_NUM,
				     csi_dev->csi_sd_pads);
	if (ret < 0) {
		media_entity_cleanup(&csi_dev->csi_sd.entity);
		video_device_release(vdev);
		goto err_vdev;
	}
	v4l2_set_subdevdata(&csi_dev->csi_sd, csi_dev);
	ret = v4l2_device_register_subdev(&csi_dev->v4l2_dev, &csi_dev->csi_sd);
	if (ret < 0) {
		dev_err(dev, "failed to register CSI subdev: %d\n", ret);
		media_entity_cleanup(&csi_dev->csi_sd.entity);
		video_device_release(vdev);
		goto err_vdev;
	}
	csi_dev->csi_sd_registered = true;

	csi_dev->vdev_pad.flags = MEDIA_PAD_FL_SINK;
	ret = media_entity_pads_init(&vdev->entity, MX6S_CSI_VDEV_PADS_NUM,
				     &csi_dev->vdev_pad);
	if (ret < 0) {
		v4l2_device_unregister_subdev(&csi_dev->csi_sd);
		csi_dev->csi_sd_registered = false;
		media_entity_cleanup(&csi_dev->csi_sd.entity);
		goto err_vdev;
	}
#endif
...
#if defined(CONFIG_MEDIA_CONTROLLER)
	ret = mx6s_csi_create_capture_link(csi_dev);
	if (ret < 0) {
		mutex_unlock(&csi_dev->lock);
		goto err_irq;
	}
...
err_irq:
#if defined(CONFIG_MEDIA_CONTROLLER)
	media_entity_cleanup(&csi_dev->vdev->entity);
	if (csi_dev->csi_sd_registered) {
		v4l2_device_unregister_subdev(&csi_dev->csi_sd);
		csi_dev->csi_sd_registered = false;
		media_entity_cleanup(&csi_dev->csi_sd.entity);
	}
