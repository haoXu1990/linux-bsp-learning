 ...
 *
 * Original single-file implementation kept for study/reference.
 * It is intentionally not compiled by the current Makefile. The active
 * implementation is split into iap_function.c, iap_io.c, iap_chardev.c
 * and iap_core.h, while still producing f_iap.ko.
 */
...
struct usb_function *iap_alloc(struct usb_function_instance *fi)
{
    ...
    // 配置管理 bind/unbind
    // gadget 注册前绑定资源(接口标识符，类描述符，端点，IO缓存)
